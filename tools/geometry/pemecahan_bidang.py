from qgis.PyQt.QtWidgets import (
    ...
    QPushButton, QMessageBox, QComboBox, QSpinBox, QDoubleSpinBox, QRadioButton,
    QGroupBox, QFormLayout, QSlider
)
from qgis.PyQt.QtCore import Qt
from qgis.PyQt.QtGui import QColor
from qgis.core import (
    QgsGeometry, QgsFeature, QgsProject, QgsDistanceArea, QgsPointXY, QgsWkbTypes
)
    ...
    def find_closest_segment_angle(self, geom, point):
        ...
        for part in parts:
            if part.wkbType() in (QgsWkbTypes.Polygon, QgsWkbTypes.Polygon25D):
                ...
                # Only looking at first poly if multi, but actually parts are polygons here
...
class PemecahanDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        self.slider_sudut.valueChanged.connect(lambda v: self.spin_sudut.setValue(v / 10.0))
        self.spin_sudut.valueChanged.connect(lambda v: self.slider_sudut.setValue(int(v * 10)))
        ...
        self.btn_custom_rasio = QRadioButton('Persentase Custom (pisahkan dengan koma)')
    ...
    def get_values(self):
        ...
        if self.btn_sama_rata.isChecked():
            ...
            try:
                parts = [float(x.strip()) for x in self.input_rasio.text().split(',')]
                if len(parts) != n:
                    raise ValueError(f"Jumlah rasio ({len(parts)}) tidak sama dengan jumlah bidang ({n})")
                total = sum(parts)
                if abs(total - 100.0) > 0.1:
                    raise ValueError("Total persentase harus 100%")
                ratios = parts
            except Exception as e:
                # We return None instead of showing warning immediately to avoid popup spam during slider drag
                return None
...
class PemecahanBidang:
    ...
    def run(self):
        ...
        if not layer:
            QMessageBox.warning(None, 'Pemecahan Bidang', 'Tidak ada layer yang aktif!')
            return

        selected = layer.selectedFeatures()
        if len(selected) != 1:
            QMessageBox.warning(None, 'Pemecahan Bidang', 'Pilih tepat 1 bidang yang akan dipecah!')
            return

        feature = selected[0]
        geom = feature.geometry()
        
        if geom.wkbType() not in [QgsWkbTypes.Polygon, QgsWkbTypes.MultiPolygon, QgsWkbTypes.Polygon25D, QgsWkbTypes.MultiPolygon25D]:
            QMessageBox.warning(None, 'Pemecahan Bidang', 'Fitur yang dipilih harus berupa Poligon!')
            return
        ...
        self.dialog.btn_pick_angle.clicked.connect(lambda: self.start_pick_angle(feature))
        
        self.dialog.btn_ok.clicked.connect(lambda: self.do_execute(layer, feature))
    ...
    def calculate_splits(self, geom, values):
        ...
        for i in range(len(ratios) - 1):
            ...
            split_part, remainder = self.split_polygon(current_geom, target_area, angle, total_area)
    ...
    def do_preview(self, layer, feature):
        ...
        for i, part in enumerate(parts):
            ...
            color = QColor(255, 0, 0, 100) if i % 2 == 0 else QColor(0, 0, 255, 100)
            rb.setColor(color)
            rb.setWidth(2)
    ...
    def do_execute(self, layer, feature):
        values = self.dialog.get_values()
        if not values:
            QMessageBox.warning(self.dialog, 'Error', 'Silakan periksa input Rasio Luasan Anda.')
            return
        ...
        if values['field_nomor']:
            base_nomor = str(feature[values['field_nomor']]) if feature[values['field_nomor']] else ""
        ...
        for i, part in enumerate(parts):
            ...
            if values['field_nomor']:
                suffix = chr(65 + (i % 26))
                new_feat.setAttribute(values['field_nomor'], f"{base_nomor}{suffix}")
        ...
        self.dialog.accept()
        QMessageBox.information(None, 'Selesai', f'Bidang berhasil dipecah menjadi {len(parts)} bagian.\nJangan lupa Simpan (Ctrl+S).')
